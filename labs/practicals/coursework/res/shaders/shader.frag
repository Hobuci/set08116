...
layout(location = 8) in vec3 fresnel_vector;
...
colour = vec4(0.0, 0.0, 0.0, 1.0);
  ...
  // Optional normal map
  vec3 finalNormal = normal; // Easy way of adding normal map if neccessary
  if(renderNormalMap == 1){
    finalNormal = calculate_normal(normal, tangent, binormal, normalMap, tex_coord);
  }
  // Calculate directional light
	...
	{
		colour += calculate_point(points[i], mat, position, finalNormal, view_dir, tex_colour);
	}
    ...
    colour += calculate_spot(spots[i], mat, position, finalNormal, view_dir, tex_colour);
	}

  // Optional reflection / refraction
